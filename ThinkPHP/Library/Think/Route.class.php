...
class Route {
        ...
        if (!empty($rules[1])) {
            foreach ($rules[1] as $rule => $route) {
                ...
                if (isset($route[2])) { // 路由参数检查
                    if (!checkOption($route[2], __EXT__)) continue;
                }
                if ($matches = self::checkUrlMatch($rule, $args, $regx)) {
                    if ($route[0] instanceof \Closure) {
                        ...
                        if (strpos($route[0], ':')) { // 存在动态变量
                            $matches = array_values($matches);
                            $route[0] = preg_replace_callback('/:(\d+)/', function($match) use(&$matches){return $matches[$match[1]-1];}, $route[0]); 
                        }
                        if ('/' == substr($route[0], 0, 1)) { // 重定向
    ...
    private static function checkOption($options, $suffix = true) {
        ...
        if(isset($options['ext'])){
            ...
            if ($suffix != $options['ext']) return false;
        }
    ...
    private static function checkUrlMatch(&$rule, &$args, &$regx) {
        ...
        if ('/' == substr($rule, 0, 1)) { // 正则路由
            ...
            $paths = explode('/', $regx);
            if ('$' == substr($rule, -1) && count($args) != count($paths)) { // $结尾则要求完整匹配
                return false;
            }
            foreach ($args as $key=>$val) {
                $var = array_shift($paths) ?: '';
                if ($val[0] == 0) { // 静态变量
                    if (0 !== strcasecmp($key, $var)) return false;
                } else {
                    if (isset($val[2])) { // 设置了过滤条件
                        if ($val[2] == 'int') {
                            // 如果值不为整数
                            if (!is_int($var+0)) return false;
                        } else {
                            // 如果值在排除的名单里
                            if (in_array($var, $val[2])) return false;
                        }
                    }
                    if (!empty($var)) {
                        $params[$key] = !empty($val[1]) ? $val[1]($var) : $var;
                    } elseif ($val[0] == 1) { // 不是可选的
                        return false;
                    }
                }
            }
     ...
     * @return [type]
     */
    static private function invoke($closure, $var = array()) {
